...
public enum SDEF {
    ...
    public static func parser(for document: XMLDocument, includeHidden: Bool = false, verbose: Bool = false) -> SDEFParser {
    ...
    public static func swiftGenerator(for model: SDEFModel, basename: String, verbose: Bool = false) -> SDEFSwiftCodeGenerator {
    ...
    public static func generateSwiftCode(from url: URL, basename: String, includeHidden: Bool = false, verbose: Bool = false) throws -> String {
        let xmlData = try Data(contentsOf: url)
...
// Note: These types are already defined in their respective files and are public
